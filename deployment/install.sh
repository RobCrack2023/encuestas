...
apt install -y python3 python3-pip python3-venv nginx postgresql postgresql-contrib nodejs npm git curl
...
echo -e "${YELLOW}4. Creando directorio de la aplicación...${NC}"
mkdir -p $APP_DIR
cd $APP_DIR
...
echo -e "${YELLOW}5. Configurando Backend (Python/Flask)...${NC}"
cd $APP_DIR/backend
...
echo -e "${YELLOW}6. Inicializando base de datos...${NC}"
# Esto se hará posteriormente mediante la API /api/init-db

echo -e "${YELLOW}7. Configurando Frontend (React)...${NC}"
cd $APP_DIR/frontend
